...
export default class FacebookChannels extends BaseController {
  ...
  facebookWebhook = async (req: FastifyRequest, reply: FastifyReply): Promise<void> => {
    ...
    this.pushToWorker({
      ...
      routing: 'worker.chatbot.messages.facebook.receive.routing',
      message: {
  ...
  replyFromN8nToFacebook = async (req: FastifyRequest): Promise<FuncResponse<object>> => {
    this.pushToWorker({
      exchange: 'worker.chatbot.messages.exchange',
      routing: 'worker.chatbot.messages.facebook.reply.routing',
      message: {
        params: req.body
      }
    });
